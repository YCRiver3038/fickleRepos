...
def AudioThr():
	while stream.is_active():
		...
		try:
			peak_index = (signal.argrelmax(fftarray_windowed, order=1))[0]
			...
			try:
				peak_index = (signal.argrelmax(fftarray_windowed))[0]
				pitch_index = np.where(fftarray_windowed == (fftarray_windowed[peak_index].max()))[0]
		...
		ifftarray = np.fft.ifft(fftarray)
