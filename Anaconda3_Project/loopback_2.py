...
def AudioThr():
	...
	conbined_array_L = (np.zeros(int(CHUNK*2))).astype(float)
	conbined_array_R = (np.zeros(int(CHUNK*2))).astype(float)
	...
	watching_array_L = np.zeros(CHUNK)
	watching_array_R = np.zeros(CHUNK)
	...
	while stream.is_active():
		...
		half_index_L = len(int(conbined_array_L/2))
		half_index_R = len(int(conbined_array_R/2))

		conbined_array_L[:half_index_L] = input_array_prev_L
		conbined_array_L[half_index_L:] = input_array_cur_L

		conbined_array_R[:half_index_R] = input_array_prev_R
		conbined_array_R[half_index_R:] = input_array_cur_R
