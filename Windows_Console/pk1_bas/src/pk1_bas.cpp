...
int readMbCode()
{
	...
	printf("\n一桁のコードは頭を0で埋めてください。");
	printf("\n入力を終わる時は、Enterキーを押したあと(Ctrl + Z)を入力してください。\n");
...
int main()
{
	...
	FdispCred = fopen("dispCredits.bat", "w");
	...
	{
		fprintf(FdispCred, "@echo off\n");
		fprintf(FdispCred, "dispCredits\n");
		fprintf(FdispCred, "PAUSE\n");
		fprintf(FdispCred, "exit\n");
		fclose(FdispCred);
		system("start \"参考情報源\" dispCredits.bat");
	}
